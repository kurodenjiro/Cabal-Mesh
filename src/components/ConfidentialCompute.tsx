...
export const ConfidentialCompute: React.FC<ConfidentialComputeProps> = ({ visible, onClose }) => {
    ...
    return (
        <motion.div
            ...
            exit={{ opacity: 0 }}
        >
            <div className="w-[800px] rounded-2xl border border-slate-200 bg-white shadow-card-lg overflow-hidden flex flex-col relative h-[500px]">
                ...
                <div className="flex-1 flex relative">
                    ...
                    <div className="w-1/2 p-6 border-r border-slate-100 flex flex-col items-start bg-nobody-mint-soft/20">
                        <div className="flex items-center gap-3 mb-8">
                            <div className="w-10 h-10 rounded-full bg-white border border-nobody-mint flex items-center justify-center text-xl shadow-card">🤖</div>
                            <div>
                        ...
                        <div className="space-y-4 w-full">
                            {lines.filter(l => l.side === 'A').map((l, i) => (
                                <motion.div key={i} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
                                    <div className="bg-white border border-nobody-mint/30 shadow-card p-3 rounded-tr-xl rounded-bl-xl rounded-tl-xl text-xs text-slate-600">
                                        "{l.text}"
                                    </div>
                                </motion.div>
                            ))}
                    ...
                    <div className="w-1/2 p-6 flex flex-col items-end bg-nobody-violet-soft/20">
                        <div className="flex items-center gap-3 mb-8 flex-row-reverse text-right">
                            <div className="w-10 h-10 rounded-full bg-white border border-nobody-violet flex items-center justify-center text-xl shadow-card">🤖</div>
                            <div>
                        ...
                        <div className="space-y-4 w-full flex flex-col items-end">
                            {lines.filter(l => l.side === 'B').map((l, i) => (
                                <motion.div key={i} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
                                    <div className="bg-white border border-nobody-violet/30 shadow-card p-3 rounded-tl-xl rounded-br-xl rounded-tr-xl text-xs text-slate-600 text-right">
                                        "{l.text}"
                                    </div>
                                </motion.div>
                            ))}
                    ...
                    {/* VS Badge */}
                    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white border border-slate-200 rounded-full w-12 h-12 flex items-center justify-center font-bold text-slate-400 text-xs z-10 shadow-card-lg">
                        VS
                    </div>
                    ...
                    {lines.some(l => l.side === 'CENTER') && (
                        <motion.div
                            ...
                            className="absolute bottom-10 left-1/2 -translate-x-1/2 bg-slate-900 text-white px-6 py-3 font-semibold text-sm rounded-xl shadow-card-lg w-max"
                        >
